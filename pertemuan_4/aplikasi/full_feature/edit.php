...
<body>
    <div class="container">
        ...
        <form action="admin.php" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= isset($edit_news) ? $edit_news['id'] : '' ?>">
            <div class="mb-3">
                <label for="judul" class="form-label">Judul</label>
                <input type="text" class="form-control" id="judul" name="judul" value="<?= isset($edit_news) ? htmlspecialchars($edit_news['judul']) : '' ?>" required>
            </div>
            <div class="mb-3">
                <label for="konten" class="form-label">Konten</label>
                <textarea class="form-control" id="konten" name="konten" rows="3" required><?= isset($edit_news) ? htmlspecialchars($edit_news['konten']) : '' ?></textarea>
            </div>
            <div class="mb-3">
                <label for="id_kategori" class="form-label">Kategori</label>
                <select class="form-select" id="id_kategori" name="id_kategori" required>
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= $category['id'] ?>" <?= isset($edit_news) && $edit_news['id_kategori'] == $category['id'] ? 'selected' : '' ?>><?= $category['nama_kategori'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                <label for="tanggal" class="form-label">Tanggal</label>
                <input type="date" class="form-control" id="tanggal" name="tanggal" value="<?= isset($edit_news) ? $edit_news['tanggal'] : '' ?>" required>
            </div>
            <div class="mb-3">
                <label for="gambar" class="form-label">Gambar</label>
                <input type="file" class="form-control" id="gambar" name="gambar" <?= !isset($edit_news) ? 'required' : '' ?>>
                <?php if (isset($edit_news)): ?>
                    <img src="uploads/<?= htmlspecialchars($edit_news['gambar']) ?>" alt="<?= htmlspecialchars($edit_news['judul']) ?>" class="img-fluid mt-2">
                <?php endif; ?>
            </div>
            <button type="submit" class="btn btn-primary"><?= isset($edit_news) ? 'Update' : 'Simpan' ?></button>
        </form>
